...
public sealed class CmdletWriteEVXEntry : AsyncPSCmdlet {
    ...
    protected override Task BeginProcessingAsync() {
        ...
        var eapObj = this.SessionState.PSVariable.GetValue("ErrorActionPreference");
        errorAction = eapObj is ActionPreference ap ? ap : ActionPreference.Continue;
        if (this.MyInvocation.BoundParameters.ContainsKey("ErrorAction")) {
            string? errorActionString = this.MyInvocation.BoundParameters["ErrorAction"]?.ToString();
            if (!string.IsNullOrWhiteSpace(errorActionString) && Enum.TryParse(errorActionString, true, out ActionPreference actionPreference)) {
