...
/// <summary>
/// Discovers, registers, and projects named event rules independently of named-event output records.
/// </summary>
public static class NamedEventCatalog {
    private sealed class RuleFactoryRegistration {
        public RuleFactoryRegistration(NamedEvents namedEvent, string logName, IReadOnlyList<int> eventIds,
            Func<EventObject, NamedEventRecord> factory, Func<EventObject, bool>? canHandle, Type? ruleType) {
        ...
        public Func<EventObject, NamedEventRecord> Factory { get; }
        ...
        lock (_initLock) {
            if (_initialized) {
                throw new InvalidOperationException(
                    "NamedEventCatalog has already been initialized. Configure() must be called before first use.");
            }
            _discoveryMode = mode;
        }
    }
    ...
    public static void RegisterRuleFactory(
        ...
        Func<EventObject, NamedEventRecord> factory,
        ...
        lock (_initLock) {
            if (_initialized) {
                throw new InvalidOperationException("Rule factories must be registered before the first named-event query.");
            }
    ...
    private static void InitializeEventRulesWithReflection() {
        var assembly = typeof(NamedEventRecord).Assembly;

        var eventRuleTypes = assembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract &&
                   (t.IsSubclassOf(typeof(EventRuleBase)) ||
                    (t.IsSubclassOf(typeof(NamedEventRecord)) && t.GetInterfaces().Contains(typeof(IEventRule)))));
...
#pragma warning restore SYSLIB0050
    ...
    /// </summary>
    public static NamedEventRecord? CreateEventRule(EventObject eventObject, List<NamedEvents> targetNamedEvents) {
        var mode = _discoveryMode;
        EnsureInitialized();
        ...
        foreach (var namedEvent in targetNamedEvents) {
            ...
            try {
                ...
                var instance = (NamedEventRecord)constructor.Invoke(new object[] { eventObject });
